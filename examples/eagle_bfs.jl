...
#    for i in 3:3        
        ...
        ei , nops, c_norm2 = UnitaryPruning.bfs_evolution(generators, parameters, PauliSum(o), ket, thresh=1e-3)
       
        e[Int(i/2)+1] += ei
        # @printf(" α: %6.4f e: %12.8f+%12.8fi\n", α, real(e[Int(i/2)+1]), imag(e[Int(i/2)+1]))
        @printf(" α: %6.4f e: %12.8f +%12.8fi nops: %8i norm2: %3.8f\n", α, real(e[Int(i/2)+1]), imag(e[Int(i/2)+1]), maximum(nops), c_norm2)
...
function run_eagle(α, o::Pauli{N}; k=5, ket_idx=0) where N
    ...
    expval, n_ops, c_norm2 = UnitaryPruning.bfs_evolution(generators, parameters, PauliSum(o), ket, thresh=1e-3)
    return expval, n_ops, c_norm2
end        

@time expval, n_ops2, c_norms2 = run_eagle(π/4, Pauli(127, X=[14,30,32], Y=[10,31], Z=[9,13,18,29,33]))
@printf(" e: %6.14f nops: %6i norm2: %3.8f\n",expval, maximum(n_ops2), c_norms2)
