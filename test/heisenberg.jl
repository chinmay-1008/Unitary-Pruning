...
function run_threshold()
    ...
    thresholds = [(1e-2)/i for i in 5:5:51]
    ener = []
    for th in thresholds
        # println(th)
        en = run(N=6, k=20, thresh=th)
        push!(ener, en)
        println(th, " ", en)
    end
    # data = DataFrame(thresholds, ener)
    # lin_fit = lm(@formula(ener ~ thresholds), (;thresholds, ener))
    # slope = coef(lin_fit)
    # println(slope)
